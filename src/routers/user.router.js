...
const { updateStatusClose } = require("../model/ticket/Ticket.model");
...
//Get all users
router.get("/getAll", userAuthorization, async(req, res)=>{
	try{
...
router.get("/", userAuthorization, async (req, res) => {
	...
	const { name, email, gender, phone, address, role} = userProf;
	res.json({
		user: {
			_id,
			name,
...
router.post("/", newUserValidation, async (req, res) => {
	const { name, company, gender, address, phone, email, password } = req.body;
...
// update user info
router.put("/update/:id", userAuthorization, async (req, res) => {
	  const {name, company, address,  role} = req.body
		const { _id } = req.params
 	  const result = await updateUserInfo({_id, name, company, address});
	try {
	  if (result._id) {
		return res.json({
		  status: "success",
	  ...
	  res.json({
		status: "error",
		message: "Unable to update the Info",
	  });
	} catch (error) {
	  res.json({ status: "error", message: error.message });
	}
...
// Get user profile for change
router.get("/popup/:_id", async (req, res) => {
	const { _id } = req.params;
